...
public class DiaLogPhim extends javax.swing.JFrame {
    ...
    public static javax.swing.JTextField txtMaPhim;
    ...
    Phim getForm() {
        ...
        phim.setID_LoaiPhim((txtTheLoai.getText()));
        ...
        phim.setGioiHanTuoi(txtGioiHanTuoi.getText());
    ...
    boolean validateForm() {
        //Mã Phim
        if (txtMaPhim.getText().trim().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập mã phim");
            txtMaPhim.setBackground(Color.yellow);
            txtMaPhim.requestFocus();
            return false;
        } else {
            //Check trùng
            for (Phim phim : p.selectAll()) {
                if (txtMaPhim.getText().equals(phim.getID())) {
                    JOptionPane.showMessageDialog(this, "Đã tồn tại mã phim này");
                    txtMaPhim.requestFocus();
                    txtMaPhim.setBackground(Color.yellow);
                    return false;
                } else {
                    //Check định dạng
                    Pattern pMaP = Pattern.compile("PH[0-9]{4}");
                    Matcher mMaP = pMaP.matcher(txtMaPhim.getText());
                    if (!mMaP.matches()) {
                        JOptionPane.showMessageDialog(this, "Mã phim nhập sai định dạng");
                        txtMaPhim.requestFocus();
                        txtMaPhim.setBackground(Color.yellow);
                        return false;
                    }
        ...
        if (txtTenPhim.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập tên phim");
            txtTenPhim.setBackground(Color.yellow);
            txtTenPhim.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pTenP = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mTenP = pTenP.matcher(txtTenPhim.getText());
            if (!mTenP.matches()) {
                JOptionPane.showMessageDialog(this, "Tên phim sai định dạng");
                txtTenPhim.setBackground(Color.yellow);
                txtTenPhim.requestFocus();
                return false;
            } else {
        ...
        if (txtThoiLuong.getText().trim().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập thời lượng phim");
            txtThoiLuong.requestFocus();
            txtThoiLuong.setBackground(Color.yellow);
            return false;
        } else {
            try {
        ...
        if (txtTheLoai.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập thể loại phim");
            txtTheLoai.setBackground(Color.yellow);
            txtTheLoai.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pTheLoai = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mTheLoai = pTheLoai.matcher(txtTheLoai.getText());
            if (!mTheLoai.matches()) {
                JOptionPane.showMessageDialog(this, "Thể loại phim sai định dạng");
                txtTheLoai.setBackground(Color.yellow);
                txtTheLoai.requestFocus();
                return false;
            } else {
        ...
        if (txtDienVien.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập diễn viên");
            txtDienVien.setBackground(Color.yellow);
            txtDienVien.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pDV = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mDV = pDV.matcher(txtDienVien.getText());
            if (!mDV.matches()) {
                JOptionPane.showMessageDialog(this, "Diễn viên sai định dạng");
                txtDienVien.setBackground(Color.yellow);
                txtDienVien.requestFocus();
                return false;
            } else {
        ...
        if (txtNSX.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập NSX");
            txtNSX.setBackground(Color.yellow);
            txtNSX.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pNSX = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mNSX = pNSX.matcher(txtNSX.getText());
            if (!mNSX.matches()) {
                JOptionPane.showMessageDialog(this, "NSX sai định dạng");
                txtNSX.setBackground(Color.yellow);
                txtNSX.requestFocus();
                return false;
            } else {
        ...
        //Ngôn ngữ
        if (txtNN.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập ngôn ngữ phim");
            txtNN.setBackground(Color.yellow);
            txtNN.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pNN = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mNN = pNN.matcher(txtTenPhim.getText());
            if (!mNN.matches()) {
                JOptionPane.showMessageDialog(this, "Ngôn ngữ phim sai định dạng");
                txtNN.setBackground(Color.yellow);
                txtNN.requestFocus();
                return false;
            } else {
                txtNN.setBackground(Color.WHITE);
            }
        }
        //Quốc gia
        if (txtQuocGia.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập quốc gia sx");
            txtQuocGia.setBackground(Color.yellow);
            txtQuocGia.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pQG = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mQG = pQG.matcher(txtQuocGia.getText());
            if (!mQG.matches()) {
                JOptionPane.showMessageDialog(this, "Quốc gia sx sai định dạng");
                txtQuocGia.setBackground(Color.yellow);
                txtQuocGia.requestFocus();
                return false;
            } else {
        ...
        //Độ tuổi
        if (txtGioiHanTuoi.getText().trim().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập giới hạn tuổi của phim");
            txtGioiHanTuoi.requestFocus();
            txtGioiHanTuoi.setBackground(Color.yellow);
            return false;
        } else {
            txtGioiHanTuoi.setBackground(Color.WHITE);
        }
        //Tình trạng
        //Ngày chiếu
        //Date today = new Date();
        if (jdcNgayChieu.getDate() == null) {
            JOptionPane.showMessageDialog(this, "Chưa chọn ngày chiếu");
            return false;
        }
        //Tóm tắt
        if (txtTomTat.getText().length() == 0) {
            JOptionPane.showMessageDialog(this, "Chưa nhập tóm tắt phim");
            txtTomTat.setBackground(Color.yellow);
            txtTomTat.requestFocus();
            return false;
        } else {
            //Check định dạng
            Pattern pTT = Pattern.compile("(?:\\P{M}\\p{M}*)+", Pattern.UNICODE_CASE);
            Matcher mTT = pTT.matcher(txtTomTat.getText());
            if (!mTT.matches()) {
                JOptionPane.showMessageDialog(this, "Tóm tắt phim sai định dạng");
                txtTomTat.setBackground(Color.yellow);
                txtTomTat.requestFocus();
                return false;
            } else {
                txtTomTat.setBackground(Color.WHITE);
            }
        }
        return true;
    }
