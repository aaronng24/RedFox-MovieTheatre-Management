...
public class PhimDAO extends RedFoxCinema<Phim, Object> {
    ...
    public void update(Phim p) {
        String sql = "update Phim set tenphim=?, thoiluong=?,gioihantuoi=?,ngaycongchieu=?,ngonngu=?, dienvien=?,quocgia_sx=?,nsx=?,tomtat=?,trangthai=?,loaiphim=? where ID_phim=?";
        XJdbc.update(sql,
                p.getTenPhim(),
                p.getThoiLuong(),
                p.getGioiHanTuoi(),
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    p.setID_LoaiPhim(rs.getString("loaiphim"));
                    list.add(p);
                }
            } finally {
